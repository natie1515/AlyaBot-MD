...
export function db.getUser(id, opt = {}) {
  if (!id) {
...
export function db.updateUser(id, field, val) {
  if (!database.users[id]) {
    db.getUser(id);
  }
...
export function db.updateChat(id, field, val) {
  if (!database.chats[id]) {
    db.getChat(id);
  }
...
export function db.getChatUser(chatId, userId, opt = {}) {
  if (!chatId) {
...
export function db.updateChatUser(chatId, userId, field, val) {
  const key = `${chatId}:${userId}`;
  if (!database.chat_users[key]) {
    db.getChatUser(chatId, userId);
  }
...
export function db.getSettings(id) {
...
export function db.updateSettings(id, field, val) {
  if (!database.settings[id]) {
    db.getSettings(id);
  }
...
export function db.getStickersPack(id) {
  if (!id) {
...
export function db.updateStickersPack(id, field, val) {
  if (!database.sticker_packs[id]) {
    db.getStickersPack(id);
  }
...
export function db.deletedb(type, ...ids) {
...
export function db.setCreate(table, identifier, field, value) {
  if (table === 'chat_users') {
