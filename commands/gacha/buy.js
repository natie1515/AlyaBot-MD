...
const normalize = (str = '') =>
  str.toLowerCase().replace(/[*_~`]/g, '').replace(/[^\w\s]/g, '').trim()
...
export default {
  ...
  run: async (client, m, args) => {
    ...
    const now = Date.now()
    if (chatConfig.adminonly || !chatConfig.gacha) return m.reply(`🌱 Estos comandos estan desactivados en este grupo.`)
    ...
    if (!reservedCharacter && m.quoted.body) {
      ...
      reservedCharacter = chatConfig.personajesReservados.find(p => quotedMessage.includes(normalize(p.name)))
    }
    if (!reservedCharacter) {
      ...
      if (claimedEntry) {
        ...
        return m.reply(`🌱 El personaje *${claimedChar.name}* ya ha sido reclamado por *${ownerName}*.`)
      }
      return m.reply(`🪶 No se pudo identificar ni reclamar el personaje.`)
    }
    const alreadyClaimed = Object.entries(chatConfig.users).find(([_, u]) => u.characters?.some(c => normalize(c.name) === normalize(reservedCharacter.name)))
    if (alreadyClaimed) {
      const [claimerId] = alreadyClaimed
      if (claimerId === userId)
        return m.reply(`🌱 Tú ya has reclamado a *${reservedCharacter.name}*.`)
      const ownerName = db.users[claimerId]?.name || claimerId.split('@')[0]
      return m.reply(`🌱 El personaje *${reservedCharacter.name}* ya ha sido reclamado por *${ownerName}*.`)
    }
    if (reservedCharacter.userId && now < reservedCharacter.reservedUntil) {
      ...
      if (!isUserReserver)
        return m.reply(`🌱 *${reservedCharacter.name}* está protegido por *${reserverName}* durante *${secondsLeft}s*`)
    }
    if (reservedCharacter.expiresAt && now > reservedCharacter.expiresAt && !reservedCharacter.user && !(reservedCharacter.userId && now < reservedCharacter.reservedUntil)) {
      const expiredTime = ((now - reservedCharacter.expiresAt) / 1000).toFixed(1)
      return m.reply(`🍒 El personaje *${reservedCharacter.name}* ha expirado hace *${expiredTime}s*.`)
    }
    const userData = chatConfig.users[userId]
    if (user.coins < reservedCharacter.value)
      return m.reply(`🌱 No tienes suficiente *${monedas}* para comprar a *${reservedCharacter.name}*.`)
    ...
    chatConfig.personajesReservados = chatConfig.personajesReservados.filter(p => p.id !== reservedCharacter.id)
